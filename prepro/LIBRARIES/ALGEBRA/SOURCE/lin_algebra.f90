...
!
!   Copyright: 1998-2006 Alberto Guardone, Marco Fossati
...
!============================================================ 
   ...
   FUNCTION  algebraic_complement(A)   RESULT(AC)

   ! Compute the matrix of algebraic complements of input matrix A
   !----------------------------------------------------------------------------
   IMPLICIT NONE

   REAL(KIND=8), DIMENSION(:,:), INTENT(IN) :: A

   REAL(KIND=8), DIMENSION(SIZE(A,1), SIZE(A,2)) :: AC
   ...
   DO i = 1, SIZE(A,1)
     DO j = 1, SIZE(A,2)
       ...
       AC(i,j) = (-1.d0)**(i+j) * minor(A, i, j)

     ENDDO
   ENDDO

   END FUNCTION  algebraic_complement





   FUNCTION  transpose(A)   RESULT(T)
   !----------------------------------------------------------------------------
   IMPLICIT NONE

   REAL(KIND=8), DIMENSION(:,:), INTENT(IN) :: A

   REAL(KIND=8), DIMENSION(SIZE(A,1), SIZE(A,2)) :: T  
   ...
   FUNCTION  diad_product(A,B)   RESULT(C)
   !----------------------------------------------------------------------
   IMPLICIT NONE
