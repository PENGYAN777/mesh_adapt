 ...
 !----------------------------------------------------------------------------
   ...
   DO c = 1, grid % Nc_d
     ...
     IF (grid % ele_type_d(m1) == 2  .AND.  grid % ele_type_d(m2) == 2) THEN
       ...
       IF (grid % j_m_d(m1) % vec(1) /= grid % j_c(1,c)  .AND.  &
           grid % j_m_d(m1) % vec(1) /= grid % j_c(2,c)) THEN

         n(1) = grid % j_m_d(m1) % vec(1)
         n(2) = grid % j_m_d(m1) % vec(2)
         n(4) = grid % j_m_d(m1) % vec(3)

       ELSEIF (grid % j_m_d(m1) % vec(2) /= grid % j_c(1,c)  .AND.  &
               grid % j_m_d(m1) % vec(2) /= grid % j_c(2,c)) THEN
 
         n(1) = grid % j_m_d(m1) % vec(2)
         n(2) = grid % j_m_d(m1) % vec(3)
         n(4) = grid % j_m_d(m1) % vec(1)

       ELSE

         n(1) = grid % j_m_d(m1) % vec(3)
         n(2) = grid % j_m_d(m1) % vec(1)
         n(4) = grid % j_m_d(m1) % vec(2)
