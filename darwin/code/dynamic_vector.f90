...
!============================================================ 
!
!   The module provide the definition of the dynamic integer 
!   vector format to be used when arrays of variable length 
!   arrays are to be stored (for example when storing the 
!   connectivity matrix of elements of variable dimension)  
!============================================================ 
...
!============================================================ 
   ...
   !------------------------------------------------------------ 
      ...
      ! print*, ' Allocation of the inverse div'
      ! -----------------------------
      ALLOCATE ( inv_div(max_idx_div-min_idx_div+1) )  

      ! print*, ' Computation of the size of each dynamic vector in inv_div(:)'
      ! ------------------------------------------------------------
      ...
      ! print*, ' Allocation and initialization of each dynamic '
      ! print*, ' vector in inv_dir(:) '
      ! ---------------------------------------------
      ...
      ! print*, ' The connectivity relation is now inverted'
      ! print*, ' The vector inv_div_vec_size is used as index '
      ! --------------------------------------------
      DO i = 1, size_DIV(div,1)  
         DO j = 1, SIZE(div(i)%vec)
            IF ( div(i)%vec(j) .NE. 0) THEN  
   ...
   !   DIV format.  Null elements of the matrix ARE stored.
   ! 
   ! INPUT:    
   !   matrix   
   ! 
   ...
   !------------------------------------------------------------ 
   !
   !   This function is used to convert a vector in DIV format 
   ...
   !               3 --> number of independent indices in div
   ! 
