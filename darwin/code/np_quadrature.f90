...
MODULE np_quadrature
  ...
  CONTAINS
   ...
   FUNCTION  np_quadr_w_u(j_c, mass_ii, mass_ij, uu)   RESULT(pp)
   !----------------------------------------------------------------------------
   IMPLICIT NONE 
   ...
   FUNCTION  np_quadr_w_Dv(j_c_d, j_c_b, jd_jb, eta, chi_b,  &
                           xi_bp, vv, vv_b)   RESULT(pp) 
   !----------------------------------------------------------------------------
   IMPLICIT NONE 
   ...
   IF ( .NOT. PRESENT(vv_b) ) THEN  ! No imposed boundary conditions
   ...
   ELSE  ! Imposed boundary conditions
   ...
   FUNCTION  np_quadr_w_Gu(j_c_d, j_c_b, jd_jb, eta, chi_b,   &
                           xi_bp, uu, uu_b)   RESULT(pp)
   !----------------------------------------------------------------------------
   IMPLICIT NONE 
   ...
   ! Boundary contribution
   IF (.NOT. PRESENT(uu_b)) THEN  ! No imposed boundary conditions
   ...
   FUNCTION  np_quadr_w_ROTu(j_c_d, j_c_b, jd_jb, eta, chi_b,  &
                             xi_bp, uu, uu_b)   RESULT(pp)
   !----------------------------------------------------------------------------
   ...
   REAL(KIND=8), DIMENSION(3, SIZE(uu,2))  ::  pp

   REAL(KIND=8), DIMENSION(2,3) :: VV
   REAL(KIND=8), DIMENSION(3) :: uu_ij
   REAL(KIND=8), DIMENSION(3) :: pp_ij
   INTEGER  ::  i, j, c, i_, j_
   !----------------------------------------------------------------------------
   ...
   DO c = 1, SIZE(eta,2)
      ...
      uu_ij(1:SIZE(eta,1)) = 0.5 * (uu(:,i)  +  uu(:,j))
 
      VV(1,1:SIZE(eta,1)) = eta(:,c)
      VV(2,:) = uu_ij
   ...
   IF (.NOT. PRESENT(uu_b)) THEN  ! No imposed boundary conditions
      ...
      DO c = 1, SIZE(chi_b,2)
         ...
         uu_ij(1:SIZE(eta,1)) = 0.5 * (uu(:,j)  -  uu(:,i))

         VV(1,1:SIZE(eta,1)) = chi_b(:,c) 
         VV(2,:) = uu_ij
      ...
      DO i_ = 1, SIZE(xi_bp,2)
         ...
         VV(1,1:SIZE(eta,1)) = xi_bp(:,i_) 
         VV(2,1:SIZE(eta,1)) = uu(:,i)
   ...
   ELSE   ! Imposed boundary conditions
      ...
      DO c = 1, SIZE(chi_b,2)
         ...
         uu_ij(1:SIZE(eta,1)) = 0.5 * (uu_b(:,j)  -  uu_b(:,i))
   ...
   END FUNCTION  np_quadr_w_ROTu




 
   FUNCTION  np_quadr_w_Hu(j_c_d, j_c_b, jd_jb, K_ij, Kb_ijs, Kb_ija, &
                           uu, uu_b)   RESULT(pp)
   !----------------------------------------------------------------------------
   IMPLICIT NONE 
   ...
   INTEGER,      DIMENSION(:,:),   INTENT(IN) :: j_c_d, j_c_b
   INTEGER,      DIMENSION(:),     INTENT(IN) :: jd_jb
   REAL(KIND=8), DIMENSION(:,:,:), INTENT(IN) :: K_ij, Kb_ijs, Kb_ija
   REAL(KIND=8), DIMENSION(:),     INTENT(IN) :: uu
   REAL(KIND=8), DIMENSION(:),     INTENT(IN), OPTIONAL :: uu_b

   REAL(KIND=8), DIMENSION(SIZE(K_ij,1), SIZE(K_ij,1), SIZE(uu)) :: pp

   REAL(KIND=8), DIMENSION(SIZE(K_ij,1), SIZE(K_ij,1)) :: pp_ij
   REAL(KIND=8) :: uu_ij
   ...
   IF (.NOT. PRESENT(uu_b)) THEN  ! No imposed boundary conditions
      ...
      ! Node-pair contribution
      DO c = 1, SIZE(Kb_ijs,3)
         
         i_ = j_c_b(1,c);  j_ = j_c_b(2,c)
         i  = jd_jb(i_);   j  = jd_jb(j_)

         uu_ij = 0.5 * (uu(j)  -  uu(i))
         pp_ij = uu_ij * (Kb_ijs(:,:,c) + Kb_ija(:,:,c))
   ...
   FUNCTION  np_quadr_Gw_nGu(j_c_d, stiff_ij, uu, nu)   RESULT(pp)
   !----------------------------------------------------------------------------
   IMPLICIT NONE 
