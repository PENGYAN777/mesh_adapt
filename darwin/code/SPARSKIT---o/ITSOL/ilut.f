...
c     implicit none
      integer n
      integer ja(*),ia(n+1),lfil,jlu(*),ju(n),jw(2*n),iwk,
     *     iperm(2*n),ierr
      real*8 a(*), alu(*), w(n), droptol
c----------------------------------------------------------------------*
...
c-----------------------------------------------------------------------
      implicit none
      integer n
      integer ja(*),ia(n+1),mbloc,jlu(*),ju(n),jw(2*n),iwk,
     *     iperm(2*n),ierr
...
c-----------------------------------------------------------------------
c          Introduction de eps absolu,  J-L Guermond 01/Avril/1997
c       subroutine pgmres(n, im, rhs, sol, vv, eps, maxits, iout,
       subroutine pgmres(n, im, rhs, sol, vv, eps, epsabs, maxits, iout,
     *                    aa, ja, ia, alu, jlu, ju, ierr)
c-----------------------------------------------------------------------
       implicit real*8 (a-h,o-z)
       integer n
       integer im, maxits, iout, ierr, ja(*), ia(n+1), jlu(*), ju(n)
...
c
 ...
 131   format(1h ,2e14.4)
       if (iout .gt. 0)
     *      write(iout, 199) its, ro
c      if (i.lt.im .and. (ro.gt.eps1)) goto 4 !J-L Guermond 01/avril/97
       if (i.lt.im .and. (ro.gt.eps1) .and. (ro.gt.epsabs)) goto 4
c
...
c
c       if (ro .le. eps1) goto 990   J-L Guermond 01/avril/97
c       if (ro .le. epsabs) goto 990  Alternativa: A Guardone 19/04/1999 
       if ((ro .le. eps1) .OR. (ro .le. epsabs)) goto 990 
